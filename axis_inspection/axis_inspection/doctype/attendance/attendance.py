...
def calculate_late_entry_early_exit_duration(self):
    ...
    if self.late_entry == 1:
        ...
        shift_time, late_entry_grace_period = frappe.db.get_value(
            'Shift Type', {'name': self.shift}, ['start_time', 'late_entry_grace_period'])
        shift_start_time = shift_time + \
            timedelta(minutes=cint(late_entry_grace_period))
    ...
    if self.early_exit == 1:
        ...
        shift_time, early_exit_grace_period = frappe.db.get_value(
            'Shift Type', {'name': self.shift}, ['end_time', 'early_exit_grace_period'])
        shift_end_time = shift_time - \
            timedelta(minutes=cint(early_exit_grace_period))
...
def calculate_delay_duration(late_entry, early_exit):
    ...
    total_duration = round(
        ((late_entry.total_seconds()+early_exit.total_seconds())/3600), 2)
